...
- **Component split:**
  ...
  - `data/sunMattersAssets.ts` — playcanvas-free helpers: `canRunSplat()`, `supportsAv1()`, `videoSrc()`, `preloadUrls(splatOk)`, `HERO_VIDEO_FULL`, `EXPERIENCE_POSTER`, `basename()`. Imported by InteractiveStage so the preloader stays light.
...
- **Runs the SAME on desktop and mobile** — deliberately NOT in the site's touch (`hover: none`) gate. `canRunSplat()` does a REAL capability probe: WebGL2 + actually building an RGBA16F float render target and checking the framebuffer is COMPLETE (the extension flag alone lies on iOS — it advertises `EXT_color_buffer_float` but can't render the float target the splat composites through → blank splat). Devices that fail → full-video fallback.
- **Two-layer mobile fallback** (guarantees mobile is never stuck on a blank splat): (1) static `canRunSplat()` probe above; (2) runtime watchdog in SunMattersExperience — if the splat is enabled but never paints `WARM_MIN` frames within `WARM_TIMEOUT` (8s), or the orchestration throws, it sets `degraded` → the effect tears down and renders the full **uncut** hero video (`HERO_VIDEO_FULL`, MP4 on Safari). 
- **Input** (`splatViewer.ts`, branches on `e.pointerType`): mouse = absolute hover-to-turn; **touch = tap-and-drag orbit** (relative from touch-down angle, `setPointerCapture`, clamped to the SAME ±5° `MAX_AZ/EL` as the mouse — full box-width drag = full range). `touch-action:none` on canvas + box so the drag doesn't scroll. Release/leave eases back to the hero pose. Hint reads "Drag to look around" on touch.
- **Video codec — Safari needs H.264.** All clips are authored AV1/WebM (Chrome/FF/modern) with an **H.264/MP4 sibling of the same basename** alongside (`seg-{a,b,c}.{webm,mp4}` + `sun-matters-bottle-scene.mp4`). iOS Safari can't decode AV1 → `videoSrc()` swaps `.webm`→`.mp4` at runtime (memoised `supportsAv1()`); both the preloader and the experience route every clip through it so blob-map keys line up. **Without the MP4, iPhone Safari = loading bar then black.**
- **Assets: `public/projects/sun-matters/splat/{serum,stick}.sog` (~7 MB each, SuperSplat SOG — PRIMARY).** Raw `{serum,stick}.ply` (~94 MB, full SH deg-3) are **gitignored** (exceed Cloudflare Pages' 25 MB file limit) — kept locally only for the dev `/splat` `.ply`↔`.sog` A/B toggle, NOT in the repo/deploy. `videos/segments/seg-{a,b,c}` cut frame-accurately at hero frames 300/700 via `scripts/cut-sun-matters-segments.sh` (inclusive ranges → boundary frames duplicated; verify with frame counts 301/401/400). Script emits both AV1/WebM + H.264/MP4 from the ProRes master.
