...
export function CursorTextNav({ activeTileId, tileRegistry, introPhase, onNavigate }: CursorTextNavProps) {
  ...
  const [isPrimaryZone, setIsPrimaryZone] = useState(true);
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      setIsPrimaryZone(isProjectMode ? event.clientY < window.innerHeight * 0.5 : event.clientX < window.innerWidth * 0.5);
  ...
  }, [enabled, isPrimaryZone, isProjectMode, nextProjectId, onNavigate, previousProjectId]);

  const label = isProjectMode ? (isPrimaryZone ? (hasPreviousProject ? "PREVIOUS" : "HOME") : hasNextProject ? "NEXT" : "HOME") : "HOME";
