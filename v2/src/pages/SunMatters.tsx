...
// Sun Matters interactive experience (segment-based).
// Each clip plays, decelerates, and ENDS on a hero frame — the browser holds
// that last frame (no playback to fight). The splat crossfades in over the held
// frame, the user parallaxes, and on "Continue" the camera homes to the hero
// pose; the NEXT clip is pre-seeked to its first frame (identical to the held
// frame) and brought up behind the splat, the splat fades out, then the next
// clip starts playing. Seamless, because every seam sits on the hero frame.
...
// Full continuous hero video — used for the mobile / no-WebGL fallback.
const HERO_VIDEO_FULL = '/projects/sun-matters/videos/sun-matters-bottle-scene.webm';

// The splat pipeline needs WebGL2 + float color buffers; touch devices get the
// straight video (matches the rest of the site's touch UX).
function canRunSplat(): boolean {
...
export function SunMatters() {
  ...
  useEffect(() => {
    ...
    // Decel is tunable live via ?decel=<sec>&minrate=<0..1> so the feel can be
    // dialed in-browser without a rebuild. Higher minrate = more abrupt stop.
    ...
    const playWithDecel = (v: HTMLVideoElement, rampUp: boolean, onApproach: () => void): Promise<void> =>
      new Promise((resolve) => {
        ...
        const loop = (): void => {
          ...
          // Warm the splat well before the decel so GL is hot by the time we stop.
          ...
          // Linear ramp-down to the floor (NOT eased — smootherstep flattens at the
          // bottom and lingers exactly where the frame-step judder is worst). Skip
          // entirely on ?hardstop: play full speed and let the clip just end.
          if (!hardStop && remaining <= decelWindow) rate = Math.min(rate, minRate + (1 - minRate) * (remaining / decelWindow));
          v.playbackRate = Math.min(16, Math.max(RATE_MIN, rate)); // clamp to browser-supported range
    ...
    const tween = (from: number, to: number): Promise<void> =>
      new Promise((res) => {
        const t0 = performance.now();
        const step = (): void => {
          if (disposed) return res();
          const k = Math.min(1, (performance.now() - t0) / FADE_MS);
          splat?.setOpacity(from + (to - from) * ease(k));
    ...
    const runClip = async (index: number, resumed: boolean): Promise<void> => {
      ...
      await loadClip(cur, clip.src);
      if (disposed) return;
      setFront(cur);
      go('playing');

      // Preload this stop's splat NOW (silent, no GL) so the ~34 MB fetch happens
      // over the whole clip — not crammed into the 1.2 s decel window. Clips
      // without a stop drop any held splat.
      ...
      // HANDOFF: pre-seek the next clip to its first frame (identical to the held
      // frame) and bring it up behind the splat, then fade the splat out.
      const next = index + 1;
      if (next < TIMELINE.length) await loadClip(nxt, TIMELINE[next].src);
      if (disposed) return;
    ...
    // kick off (catch load failures — e.g. a segment still encoding)
    ...
    return (
      <div style={{ position: 'fixed', inset: 0, background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <video
          ...
          style={{ width: 'min(100vw, calc(100vh * 16 / 9))', aspectRatio: '16 / 9', objectFit: 'fill' }}
        />
      </div>
    );
  }
